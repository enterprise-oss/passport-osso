 ...
 *         callbackURL: 'https://www.example.net/auth/osso/callback'
 *       },
...
function Strategy(options, verify) {
  ...
  options.authorizationURL =
    options.authorizationURL || 'https://accounts.osso.com/authorize';
  options.tokenURL = options.tokenURL || 'https://accounts.osso.com/api/token';
  options.scopeSeparator = options.scopeSeparator || ' ';

  OAuth2Strategy.call(this, options, verify);
  this.name = 'osso';
  this._userProfileURL = options.userProfileURL || 'https://api.osso.com/v1/me';
 ...
 *
 *   - `provider`         always set to `osso`
 *   - `id`               the user's Osso ID
 *   - `username`         the user's Osso username
 *   - `displayName`      the user's full name
 *   - `profileUrl`       the URL of the profile for the user on Osso
 *
  ...
  this._oauth2._request('GET', this._userProfileURL, headers, '', '', function (
    ...
    try {
      ...
      var profile = {
        ...
        profileUrl: json.external_urls ? json.external_urls.osso : null,
        photos: json.images
